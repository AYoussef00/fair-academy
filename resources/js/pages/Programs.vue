...
import { BookOpen, Clock3, Layers } from 'lucide-vue-next';
...
<template>
    <Head title="البرامج – أكاديمية فايرير للتدريب والتعليم" />
    <div class="min-h-screen bg-slate-50 text-slate-900">
        <SiteHeader />

        <section class="border-b border-slate-200 bg-white">
            <div class="mx-auto max-w-7xl px-4 py-12 sm:px-6 sm:py-16">
                <p class="inline-flex items-center gap-2 rounded-full bg-emerald-100 px-4 py-1.5 text-xs font-semibold text-emerald-700">
                    <Layers class="h-4 w-4" />
                    برامج تدريبية متكاملة
                </p>
                <h1 class="mt-4 text-3xl font-extrabold sm:text-4xl">البرامج</h1>
                <p class="mt-3 max-w-2xl text-sm leading-relaxed text-slate-600 sm:text-base">
                    تصفح البرامج المنشورة على المنصة واختر المسار الأنسب لك.
                </p>
            </div>
        </section>
        ...
        <section class="mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-14">
            ...
            <div v-else class="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                ...
                >
                    <Link :href="`/programs/${program.slug}`" class="block">
                        <ImageWithFallback
                        ...
                        <div class="space-y-3 p-5">
                            <div class="flex flex-wrap items-center justify-between gap-2 text-xs">
                                ...
                                <span v-if="program.duration" class="inline-flex items-center gap-1 text-slate-500">
                            ...
                            <p v-if="program.description" class="line-clamp-2 text-sm text-slate-600">
                                {{ program.description }}
                            </p>
